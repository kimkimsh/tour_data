...
| 헤리티지 | 국가유산청 OpenAPI (`cha.go.kr/openapi`) | F2 어른 모드, F4 공식 해설 |
...
- **데이터활용 20점:** 10개 KTO 서비스가 각각 독립 기능에 연결되는 게 아니라 `accessibility_facts`라는 **단일 정규화 테이블로 수렴**한 뒤 F1–F5로 파생된다. 심사위원이 데이터 흐름을 한 슬라이드로 이해할 수 있다.
