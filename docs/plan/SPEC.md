...
## 14. Plan-review revisions v2 (2026-06-15) — sixth-pass resolutions

> Source: a sixth independent review pass — a Claude 6-lens workflow (CEO/Eng/DevEx/Design/office-hours/consistency) + 8 live 2026 web re-verifications, run in parallel with an independent Codex `gpt-5.5`/`xhigh` review. Full findings: [`_research/_plan_review_v6_findings.md`](./_research/_plan_review_v6_findings.md) and [`_research/_codex_review_v6_raw.md`](./_research/_codex_review_v6_raw.md). The two model families converged on the same top blockers (capability_code fragmentation, coverage-label nondeterminism, ETL revalidation/publish drift, unauditable F1→F5 lineage) — that convergence is the confidence signal. Calibration (user, 2026-06-15): KEEP the §13.2 scope cuts; invest the generous runway in DEPTH + VALIDATION; SPEC.md is canonical. Locked numerics (§2.14, §7) stand; the two items that touch locked behavior (14.3, 14.4) are user-adjudicated.

**14.1 SPEC authority (single canonical file).** `SPEC.md` is the only authoritative spec. `SPEC.ko.md` is a non-authoritative Korean translation that defers to it; on any conflict `SPEC.md` wins. Add a translation-drift check.

**14.2 `capability_code` single vocabulary (lineage join key) — BLOCKER.** `16_suitability_policy.md §2` is THE single authority for every `capability_code` string. The ETL normalizer (`04`), DB catalog (`03`), F1 mapping (`05`), F5 field list (`09`), and all tests (`13`) MUST use those exact strings — no `entry.wheelchair` / `MOBILITY_WHEELCHAIR_ENTRY` / `BF_BRAILLE_BLOCK` / raw-`wheelchair` variants. No stream freezes until a CI **set-equality** test proves `{ETL emits} = {policy catalog} = {domain switches on} = {F5 reads}`, plus one contract test tracing a single capability KTO→F1→F3→F5 with the same code at every hop.

**14.3 coverage<0.65 label (USER DECISION — touches §7/§16).** Two-tier rule that preserves both prior guards: a selected-persona `critical` capability = `unknown` → `정보없음`; criticals known but overall `coverage < 0.65` → label capped at `주의` (never `방문가능`). `16 §9` step 2 drops "OR coverage<0.65" (critical-unknown only); the coverage<0.65 case is owned by the `16 §6` 주의-cap. Golden cases assert exactly one label per fixture. (`unknown=0.35`, C `+0.12`, D decay unchanged.)

**14.4 UGC submission auth (USER DECISION — preserves §2.11).** Anonymous (Supabase Anonymous Auth) identities MAY submit F3 barrier reports; §2.11 stands. Remove `07`'s RESTRICTIVE "permanent users only" insert policy and the matching `07 §10.2` row; abuse is controlled by the already-specified guards (server rate-limit, file size/MIME, duplicate-hash, reporter-trust, admin moderation/MFA, audit retention). `03`/`07` RLS and the `is_anonymous` mirror must agree that anonymous-auth identities insert their own reports.

**14.5 Contract-drift unification (one authority per concern).** (a) ETL revalidation: one HMAC convention — sign `body + timestamp`, header `x-etl-signature`, env `ETL_HMAC_SECRET`; delete the Bearer path; KTO key env = `KTO_SERVICE_KEY_DECODING`. (b) Publish atomicity: `03`'s `publish_dataset()` delete-then-insert is authoritative; delete `02`'s `published_version` read-model filter and reconcile `dataset_versions` to one shape; fix migration order so staging is created after its base table. (c) Per-POI cache: F2 docent loader uses `docent:{poi}:{locale}` tags (not static `['docent','poi']`); the daily ETL emits only registry tags. (d) Workspace scope is `@modu/*` everywhere. (e) `buildDiaryDocument` stays pure-domain returning a document DTO; PDF/BRF/GPX rendering lives in `packages/exports` (CX), called by features — not by the domain function. (f) Single `absence` enum, single docent-channel enum, single `PersonaId` domain matching the DB `persona_code` enum — declared once, imported elsewhere. (g) Domain tests = vitest; contract/exports/component = jest — stated per package with a named owner for each config.

**14.6 F1→F5 lineage must be auditable (the win condition) — BLOCKER.** Add a lineage manifest (not a new user feature): `source_record_id`, `dataset_version`, `fact_id`, `evidence_id`, `policyVersion` propagated into F2 and F4 outputs (today they share nothing with F1). Ship one golden-flow assertion + a judge-facing "data passport" exhibit showing the identical `source_field`/`verified_at`/evidence-pack id surfacing in F1.A, F1.B, F3, F4, F5 for one capability (recommend 부소산성 elevator). The §12/PT closing diagram includes F2.

**14.7 F5 honesty + 데이터활용 framing.** F5 completeness denominator = the 21 scored capabilities (exclude the 4 `*etc` fields) so AC-F5-06 (F5 null == F1 정보없음) can hold; add a contract test. Label the dashboard "6 POI 기준 갭 — 동일 스키마로 전 시군 확장(발전방향)" with a visible "검증 POI 수" denominator; per-POI rows, not absolute 시군 counts. Reconcile PT 샷6 to the single GapPriorityTable (the cut heatmap/visitor-chart/report-frequency are NOT live). Reframe the 데이터활용 headline from "10 services converge into one table" (false — only `detailWithTour2` writes `accessibility_facts`) to a **hub-and-spoke on `poi_id`**: one authoritative accessibility source + typed context layers (Odii/TatsCnctr/DataLab/PhotoGallery/TarRlteTar) keyed off the same POI. This is a stronger, "show-me"-proof story.

**14.8 Verify-then-seed the hero POI; observe demand.** Run the 공산성 서문 wheelchair on-site traversal check FIRST; never seed `wheelchair_access=supported` before verification; name 국립공주박물관 as the fallback hero if it falsifies. In the same field trip, run 5–8 real 관광약자 interviews to convert the asserted "5 questions" demand into observed pain with verbatim quotes. Pilot one real F3→moderation→fact-refresh→F5-gap-delta instance so the 발전성 flywheel is observed, not asserted.

**14.9 Current-fact corrections (2026 web-verified).** Odii `langCode` is `ko/en/cn1/jp` (NOT `ja/zh-CN`) → add `ODII_LANGCODE_MAP {ja:jp, zh-CN:cn1}`, fix `getDocentStories`, probe GATE-5 with `jp/cn1`. Odii `xCoord/yCoord` CRS/axis/units unconfirmed → verify against v4.1 before seeding; derive one coordinate source from verified POI geometry; reconcile the two seed tables that disagree by >radius. `TatsCnctrRateService` is a **30-day forward ML forecast**, not current crowding → relabel "예측 혼잡도(향후 30일)" + a `predicted` provenance flag (no formula change). KTO operating account is **manual 심의** (not auto); 100,000/day is an advertised max requiring 활용사례 registration + a separate increase request (two sequential 심의, SLA 2–3일); apply **per dataset**, early. Multilingual `contentTypeId` 12→76 / 14→78 remap added to the `getMultilingual` request contract + a pure exhaustive mapper. `braillify` 2.0.1 exports `encode`/`translateToUnicode`/`translateToBrailleFont` (NOT `translate`/`braillify`) — fix both import sites; BRF uses a Korean-braille-aware transcriber with an explicit target profile, validated plaintext→braille→BRF→embosser with signed reader approval. Pretendard has no Hanja glyphs → transliterate 史477/史13/史5/史301 or add a Noto Sans KR fallback; add a "한자 깨짐 0" AC. `react-pdf` font registration uses `process.cwd()` + `outputFileTracingIncludes` (not a web-absolute `/fonts/...` path), preloads font Buffers before render; `pdf-lib` sets `subset:false`; `braillify` WASM needs `asyncWebAssembly` + tracing or runs in ETL with the asset stored. 문화재청 → 국가유산청, `cha.go.kr` → `khs.go.kr` (or data.go.kr `15034324`); pin one license per endpoint. AI 기본법: the grace period defers only 과태료/사실조사, not labeling (effective now); add §31(1) advance notification (onboarding/ToS) and embed an in-document AI-source notice in externally-distributed PDFs/BRF (§31(2)). 위치정보법: add the 2024 소상공인/1인-창조기업 simplified/deferred-notification path + an eligibility determination; resolve the §1.1-vs-§1.5 사업자-classification contradiction; relax the hard 8/31 deadline accordingly. Next.js: `use cache` with Cache Components off is a hard **build error** (not a no-op) — reword the guard; `vercel.json` functions globs are relative to the Vercel Project Root (`apps/web`). Infra: pin `@supabase/ssr` 0.12.0 + `@serwist/next` 9.5.11; Supabase region `ap-northeast-2` is immutable after create + Free pauses after 7 idle days (keep-alive = a real SELECT); Vercel Pro (icn1 Seoul residency) is **launch-blocking**. Kakao Map free 300k/Local 100k daily, overage 0.1/0.5/2 KRW ("80% discount" is Talk-Share only); `juso` 좌표제공 needs scheduled approval (named owner); slope uses **NGII open DEM** (VWorld DEM is security-restricted); Kakao Mobility walking-directions is partner-gated and OUT. Strike the stale global "areaCode 2025-12-31 cutoff" claim in `_research/D1` (deprecation is rolling per-service, 90-day) + a CI grep guard against `areaCode` write filters.

**14.10 Depth/validation + DevEx hardening (the generous-runway investments).** KWCAG: cite canonical 33-item KWCAG 2.2 IDs (not raw WCAG numbers like 1.2.6/4.1.3/2.4.3) + a CI lint on citation strings; single-source `AiBadge` (one variant enum + path) and fix its KWCAG 2.5.3 accessible-name violation; reconcile the `wcag22aa` PR-gate policy + an RC-blocking manual checklist for the 9 KWCAG-2.2-new items; use OpenWAX + the KWCAG 2.2 표준 점검 지침 (K-WAH is a 2.0-era tool). Validation: pre-register an audit-grade protocol (independent assessors; persona/user strata including deaf/KSL + braille users; calibrated measurement; inter-rater agreement; signed raw records; severity rules; remediation deadlines; regression retest). Procurement: one dependency register with a NAMED accountable person + request-by/need-by dates + acceptance artifact + licensing + fallback for 충남교육청 form, mascot art, braille partner, **CACF LOI (relationship owner + pre-drafted LOI + parallel track to 다도라/올담 or 충남교육청 + a hard auto-downgrade date)**, KTO operating applications (per dataset), CLOVA/ElevenLabs keys. Design (new lens): spec the multi-persona selection UX (warm template chips + visible `min()` "strictest member" framing — the 기획력-30 differentiator currently has no UI); spec the cross-feature Empty/`정보없음`/zero-result/first-run states (make the common state the visible start of F1→F3→F5, not a dead end); re-tier the 4-Layer card **verdict-first** ("can MY group go + the one thing to watch" before the A×B×C×D math). Tests: replace the whole-file PDF SHA-256 golden with a normalized hash (fixed dates/producer); golden-generate the F5 worked-example numbers (ban hand-authored numbers as F1 already does); make the demo-resilience E2E exercise the snapshot read-model path (the browser never calls KTO, so aborting KTO in-browser proves nothing). DevEx: add a day-1 setup runbook (clone→install→`supabase start`→`db reset`→seed→first green test) + `.env.example` + `.nvmrc`; promote the scripts list to a complete authoritative manifest (define `etl:ingest`/`capture-fixtures`/`exports:update-golden`; reconcile `start:test` vs `start:demo`); add a half-page first-slice fixture spec (exact personas/budget, the 3 named route steps, the one F5 gap row, expected label) and pull a minimal CX `renderHtml` into Window 1 (the slice's HTML-diary leg currently has no W1 producer).

**14.11 Status.** Direction and architecture remain APPROVED. No stream starts until **14.2** (capability vocabulary) and **14.5** (contract unification) are green; the **14.6** lineage exhibit is the first-slice acceptance artifact; **14.8** hero-POI verification precedes any `supported` seed. All v6 items are contract-unification, framing-honesty, or depth/validation fixes — none restore cut scope.
