...
|---|---|
| 비용 | 전 서비스 무료 |
| **개발계정** | **일 1,000건** — 매뉴얼 원문 그대로: "개발계정은 일 1,000건의 트래픽을 제공합니다." **오퍼레이션 단위가 아니라 서비스(신청) 단위다.** 자동승인, 10~30분 후 사용 가능 |
| **운영계정** | 포털 승인 요청 후 한국관광공사 담당자 승인. 트래픽 증액은 별도 신청 **[미확인]** — 이 10종 매뉴얼에는 운영계정 서술이 없다(관광채용정보 매뉴얼에만 "일 100,000건" 언급) |
| 신청 단위 | **데이터셋(서비스)별로 각각 신청.** `KorService2`와 `KorWithService2`는 별개 |
| 유효기간 | **[미확인]** — 매뉴얼에 서술 없음. 포털에서 확인 |

**계산:** 관광지 6곳 기준 1회 전체 수집이 **약 95건**이다(§5). 한도가 **서비스 단위 1,000건/일**이고 한 서비스의 최대 사용량이 1회당 28건(Odii)이므로, 하루에 여러 번 돌려도 여유가 크다. 운영계정 없이 개발·심사를 마칠 수 있다.

> **[정정] 두 가지를 바로잡았다.**
> 1. 이전 문서(그리고 이 문서의 앞선 판)의 **"오퍼레이션당 1,000건"은 매뉴얼에 없는 말이다.** 원문은 `개발계정은 일 1,000건의 트래픽을 제공합니다` 한 줄뿐이고 10종 매뉴얼이 전부 같다. 오퍼레이션당으로 읽으면 예산을 10배 과대평가하게 된다.
> 2. "운영계정 100,000건/일"도 이 10종 매뉴얼에는 없다. 근거 없이 쓰지 않는다.
>
> **이 정정이 실제로 바꾼 것:** 관광사진 48콜 페이징(§2.7)이 서비스 한도의 5%가 아니라 **`PhotoGalleryService1` 한도의 5%**였다는 계산은 여전히 맞지만, 그 페이징 자체가 §2.7의 정정으로 불필요해졌다.
...
- data.go.kr 데이터셋 ID `15101897` **[미확인 — 포털 메타데이터이고 매뉴얼에 없다]**, 매뉴얼 **v4.3 (2025-05-12)**, 갱신 일 1회
...
> **[정정] 이전 스펙 전체가 `xCoord` / `yCoord`라고 적고 있는데 틀렸다.** 매뉴얼 v4.1은 `mapX` / `mapY`다. 그대로 짜면 모든 호출이 빈 응답이 된다. 이전 스펙의 관련 수용 기준도 함께 폐기한다.
...
> **★ `numOfRows`를 반드시 크게 준다.** 응답은 **관광지 1곳당 향후 30일 × 1행**이다(매뉴얼 XML 예시가 같은 관광지에 `baseYmd` 30건). 기본값 10으로 부르면 **30일 중 10일만 받는다.** `numOfRows=100` 이상으로 부르고 `tAtsNm`으로 거른다.
...
> **중요:** **지역 필터 파라미터가 없다.** 기간만 주면 **전국 전체**가 온다 — 기초 기준 **하루 약 740행**(시군구 229개 × `touDivCd` 3구분). 공주·부여만 쓰려면 **응답을 받아서 `signguCode`로 걸러야 한다.**
...
**시도/시군구 합산 금지와 약 4일 지연도 매뉴얼에 없다 [미확인].** 매뉴얼이 주는 것은 `데이터 갱신주기 일 1회`뿐이다. 지연 일수는 **탐침으로 실측한다** — 오늘 날짜로 호출해 응답이 오는 가장 최근 `baseYmd`를 확인하고 그 값을 쓴다. 잠정값은 `endYmd = 오늘 − 4일`.
...
**한도는 서비스(신청) 단위 일 1,000건이므로 서비스별로 나눠서 센다** (§1.6).

| 서비스 | 오퍼레이션 | 호출 수 | 한도 대비 |
|---|---|---:|---|
| **KorWithService2** | `detailWithTour2` 6 | **6** | 0.6% |
| **KorService2** | `detailCommon2` 6 + `detailIntro2` 6 + `detailImage2` 6 + `areaBasedList2` 4 + (최초 1회) `ldongCode2` 1 + `lclsSystmCode2` 1 | **24** | 2.4% |
...
| **TarRlteTarService1** | `areaBasedList1` 2 | **2** | 0.2% |
| **DataLabService** | `locgoRegnVisitrDDList` 8 | **8** | 0.8% |
| **합계** | | **약 95건** | 최대 사용 서비스가 2.8% |

**어느 서비스도 한도의 3%를 넘지 않는다.** 하루에 여러 번 돌려도 여유가 크고, 운영계정 없이 개발·심사를 마칠 수 있다.

> **이전 판(약 126건)에서 줄어든 이유:** 관광사진 전수 페이징 48콜이 `gallerySearchList1` 키워드 7콜로 바뀌었다(§2.7). Odii는 키워드 추측 6콜 대신 전수 열거 16콜로 늘렸다 — 콜은 늘지만 "키워드가 맞았는지"라는 불확실성이 사라진다.
