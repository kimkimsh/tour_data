...
>
> 1. **처음:** 부정 패턴에 범용 `없음`이 있었다. `단차 없음`·`장애물 없음`·`문턱 없음`이 전부 `unsupported`로 찍혔다. 갈 수 있는 곳에 「다른 곳 권장」이 붙었다.
> 2. **그다음:** ①을 고치려고 범용 `없음`을 아예 빼자, 이번에는 **`엘리베이터 없음`·`장애인 화장실 없음`·`보조견 동반 불가`가 `unknown`이 됐다.** 확인된 장벽이 「확인 필요」로 표시됐다.
>
> **두 오류의 비용은 대칭이 아니다.** ①은 갈 수 있었던 곳을 놓치는 것이고, 화면에 원문이 있으니 사용자가 뒤집을 수 있다. ②는 **확인된 장벽을 「모른다」고 말해서 확인하러 가게 만드는 것**이고, 도착해서야 못 들어간다. 휠체어 이용자에게 잘못된 판정을 보여주는 것이 최악이라는 §1의 순위대로라면 **②를 더 무겁게 봐야 한다.**
>
> **고친 방향:** 부정어의 극성을 **문장 전체가 아니라 명사 옆에서** 읽는다. 장애물 명사(`단차`, `계단`, `돌길`…) 옆의 부정은 좋은 소식, 시설 명사 옆의 부정은 나쁜 소식이다. `경사`에 `(?!로)` 부정 전방탐색이 붙은 이유도 같다 — `경사로`는 장애물이 아니라 시설이고, 이걸 놓치면 `출입구까지 경사로가 설치되어 있음`이 「없음」으로 뒤집힌다.
>
