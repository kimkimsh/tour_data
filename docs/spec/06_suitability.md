...
> **왜 명시하는가:** 기존 스펙은 "세부 가중치는 정책 JSON에 있다"고 썼는데 **그 JSON은 작성된 적이 없다.** 31개 항목의 세부 가중치를 지금 지어내는 것보다 균등이 낫다 — 근거 없이 어떤 항목이 다른 항목보다 1.5배 중요하다고 주장할 수 없기 때문이다. **항목의 중요도 차이는 Layer B(페르소나 등급)에서만 반영한다.** 이게 A와 B의 역할 분리이기도 하다.
...
**23건** — 아래 17건 + [`09_test_and_ci.md`](./09_test_and_ci.md) §2의 6건. 기존 스펙은 30건 이상 + 전문가 서명을 요구했지만, 23건으로 모든 분기를 덮을 수 있다.
...
| 11 | 골든 케이스 30건 + 전문가 서명 전 배포 금지 | **23건**, 전문가 검수는 "미실시"로 화면에 표시 | 서명해 줄 전문가가 없다. 게이트를 지키지 못할 바엔 **정직하게 표시하는 것**이 낫다 |
...
### 10.4 현재 카탈로그에서 빠진 항목 4개 — 기록만 남긴다

원 정책의 25개 카탈로그에 있었고 현재 31개에 없는 것.

| 항목 | 원 등급 | 왜 빠졌나 | 되살릴 가치 |
|---|---|---|---|
| **`visual_alarm` 시각 경보기** | P2b critical | 대응하는 KTO 필드가 없다 | **있다.** 청각장애 사용자에게 화재·비상 시 시각 경보기는 실재하는 안전 항목이고, `curated-facts.json`으로 채울 수 있다. 청각장애 필수 항목이 현재 2개뿐인 것을 보완한다 |
...
> `visual_alarm`은 **여유가 생기면 추가할 첫 번째 항목**이다. 추가 비용은 카탈로그 한 줄 + 등급표 한 줄 + `curated-facts.json` 조사이고, 청각장애 판정의 근거를 2개에서 3개로 늘린다.
