...
```
score = round( clamp( 100 × A × B , 0, 100 ) )
...
## 5. 공식 인증 — 점수가 아니라 사실로 표시한다

**앞 판에는 `× C`(인증 보정, 1.00~1.12)가 식에 있었다. 지웠다 (2026-09-07, [`../work_log/04_open_items.md`](../work_log/04_open_items.md) 결정 3).**

지운 이유는 두 가지이고, 둘 다 실제 데이터를 모으고 나서야 보였다.

**첫째, 6곳 중 1곳에만 걸렸다.** 무령왕릉의 열린관광지 지정 하나뿐이고 효과는 54점 → 57점, 라벨은 「주의」 그대로였다. 규칙이 살아 있는데 6분의 1에만, 그것도 한 종류로만 작동하는 상태였다.

**둘째, 이게 더 중요한데 — 찾은 인증이 전부 부속 건물 것이었다.** 방문자센터, 화장실 같은 건물에 붙은 인증이다. 그대로 넣으면 **「화장실이 인증받았으니 관광지 점수를 올린다」**가 된다. 이 서비스가 하지 말자고 만든 종류의 일이다.

추가로 등급도 맞지 않았다. 공개 자료는 「예비 / 본」 둘로 구분하는데 스키마는 「예비 / 일반 / 우수」 셋을 원한다. 「본 인증」이 어느 쪽인지 알 방법이 없다.

**지금 하는 것:** `content/certifications.json`은 그대로 유지하고, 관광지 화면 머리에 **출처를 붙여 한 줄로 표시한다.** 점수에는 들어가지 않는다.

> **사실을 버린 것이 아니라 산식에서 뺀 것이다.** 무령왕릉이 열린관광지라는 것은 확인된 정보이고 방문자에게 쓸모가 있다. 다만 그것이 **얼마나** 점수를 올려야 하는지에 대해 근거가 없었고, 근거 없는 숫자를 곱하느니 사실을 그대로 보여주는 편이 이 서비스의 방식에 맞다.

**되살리려면 무엇이 필요한가** — 나중에 관광지를 늘려 인증 데이터가 의미 있는 규모가 되면, 그때 필요한 것은 (1) 인증이 **관광지 전체**에 대한 것인지 부속 건물에 대한 것인지 구분하는 필드, (2) 「본 인증」을 일반/우수로 가르는 확인된 근거, (3) 보정폭 +0.02~+0.08의 근거. 셋 다 없이 되살리면 같은 자리로 돌아온다.
...
**조정하지 않는 것:** `unknown = 0.35`, 신선도 감쇠값(`1.00 / 0.90 / 0.75`). 이 둘은 그대로 둔다. **신선도 값은 점수에 영향을 주지 않으므로 임계값 조정과 무관하다.**
...
| 16 | `score = 100 × A × B × C × **D**` (신선도 포함) | **`score = 100 × A × B × C`** (당시). 신선도는 `evidenceConfidence`에만 | §6.3이 *"오래되거나 비어 있는 것은 불확실이지 부적합이 아니다"* 라고 선언하면서 식이 그 선언을 어기고 있었다. 같은 사실이 점수와 신뢰도에 두 번 반영되기도 했다. **`layerD` → `freshness`로 이름도 바꿨다** — 레이어가 아니게 됐다. 결정: [`11_open_items.md`](./11_open_items.md) **DEC-9** |

| 17 | `score = 100 × A × B × **C**` (인증 보정 포함) | **`score = 100 × A × B`.** 인증은 관광지 화면에 사실로 표시 | 6곳 중 1곳에만 걸렸고(무령왕릉 54→57, 라벨은 그대로), 찾은 인증이 전부 방문자센터·화장실 같은 **부속 건물** 것이라 그대로 넣으면 건물의 인증으로 관광지 점수를 올리게 된다. 등급도 공개 자료(예비/본)와 스키마(예비/일반/우수)가 맞지 않았다. §5에 되살리는 조건까지 적어 뒀다. 결정: [`../work_log/04_open_items.md`](../work_log/04_open_items.md) 결정 3 |

**정책 버전:** `suitability-v5`. (v4에서 인증 보정 제거) 모든 결과에 `policyVersion`으로 붙는다. 값이 바뀌면 버전을 올리고 골든 파일을 재생성한다.

> **v4 → v5로 점수가 바뀐 곳은 무령왕릉 하나뿐이다** (57 → 54, 라벨 「주의」 그대로). 골든 27건 중 인증이 있던 2건은 규칙과 함께 삭제했고, **나머지 전부에서 점수·라벨이 한 자리도 바뀌지 않았다** — 전부 `C = 1.00`이었기 때문이다.
