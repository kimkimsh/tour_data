...
**이 항목은 철회한다.** 당시 판단은 「Chrome이 `@page` 마진 박스를 구현하지 않는다」였고, 그건 지금 사실이 아니다. Chrome 131이 마진 박스를 실었고, 이 저장소의 Chromium 151로 다시 재어 확인했다 — `@bottom-center { content: counter(page) " / " counter(pages) }`가 5쪽짜리 문서의 다섯 쪽 모두에 찍히고, `content: var(--print-attrib)`도 동작한다. 마진 박스는 `@page`의 여백 안쪽에 그려지므로 본문과 겹칠 수 없다.

`position: running()`은 여전히 없다. 하지만 마진 박스가 있으면 필요하지 않다.

**코드 대응(현재):** `@page`의 `@bottom-left`가 출처 한 줄, `@bottom-right`가 `counter(page) / counter(pages)`를 찍는다. 출처 문구는 `--print-attrib`으로 루트 요소에 실린다 — 마진 박스는 루트 요소에서만 상속받는다.

**그 전 대응이 남긴 결함:** 반복 출처를 `position: fixed` 띠로 넣어 두었는데, 인쇄에서 `position: fixed`는 **페이지 여백 안쪽**을 기준으로 잡힌다. 아래 여백을 28mm로 넓혀도 띠가 본문과 함께 내려갈 뿐이라, A4 한 쪽에서 본문 마지막 줄과 **1.25pt 겹쳤다**. 실측 좌표는 `BODYLINE046 yMax 753.13` / `FOOTERATTRIBUTIONSTRIP yMin 751.88`.

**수정할 곳:** `08_accessibility_legal.md` §1.1은 **원래 지시가 맞았다.** 고칠 것이 없다.
...
## T. 개인정보 보호책임자는 **성명 또는 부서 명칭** 중 하나면 된다 **[원문 확인]**

`13_legal_citations.md` §3.1 표의 12번이 「개인정보 보호책임자 — **이름과 연락처**」다. 조문은 둘 중 하나를 고르게 한다.

> **개인정보 보호법 제30조 제1항 제6호**
> 제31조에 따른 개인정보 보호책임자의 **성명 또는** 개인정보 보호업무 및 관련 고충사항을 처리하는 **부서의 명칭**과 전화번호 등 연락처

「또는」이 성명과 부서 명칭 사이에 있고, 「전화번호 **등** 연락처」이므로 연락 수단도 전화번호로 한정되지 않는다. 즉 **본명 없이 부서 명칭 + 전자우편으로 요건을 채울 수 있다.**

표대로 「이름과 연락처」로 읽으면 1인 운영자는 본명을 공개하는 것 외에 선택지가 없다고 판단하게 된다. 실제로 이 항목은 그 이유로 세 회차 동안 비어 있었다.

`/privacy` §12는 **고충 처리 부서 「모두의 백제 운영」 + 전자우편**으로 채웠다.

덧붙여, 지정 의무 쪽은 별개다 — **제31조 제1항 단서**가 소상공인 기준에 해당하면 보호책임자를 지정하지 않아도 된다고 하고, **제2항**이 그 경우 사업주 또는 대표자가 보호책임자가 된다고 정한다. 지정을 면제받아도 제30조 제1항 제6호의 **기재 의무는 그대로 남는다.** 둘을 같은 것으로 읽지 말 것.

> **[미확인]** 공모전 제출 요건이 처리방침에 무엇을 더 요구하는지는 공고문 소관이고, 대조하지 않았다.

---

## U. `06_suitability.md` 전체가 아직 v5다 — 코드는 v6이다 **[실측 · 스펙에 직접 반영 일부] ★**

이 문서는 3행에서 **「이 문서가 모든 수치의 단일 권위다」**라고 선언한다. 그런데 636행은 `정책 버전: suitability-v5`이고, 코드는 `POLICY_VERSION = 'suitability-v6'`이다. 그 사이에 v6이 바꾼 것들이 전부 들어 있다 — [`11_redesign_map_scoring_pois.md`](./11_redesign_map_scoring_pois.md) §2에 기록돼 있지만, **그 회차는 스펙을 고치지 않았고 스펙 쪽에도 그 회차를 가리키는 줄이 없다.**

권위라고 선언한 문서가 코드와 어긋나 있으면, 다음 사람이 코드 쪽을 위반으로 읽는다. 어긋난 지점은 이렇다.

| 스펙 | 스펙이 말하는 것 | 코드 |
|---|---|---|
| §3 상태값 표 | `unknown` → **0.35**, 분모에 남음 | `STATUS_VALUE`에 `unknown` 키가 없다. 모든 분모에서 빠진다 |
| §6.4 규칙 4b | `coverage < 0.65` → 라벨 상한 「주의」 | 삭제됨. 규칙 3이 같은 질문에 더 정확히 답한다 |
| §4.1 P0 | 판정 대상 = 32개 항목 전부 | `GENERAL_VERDICT_CODES` 4개 |
| §3 항목 수 | **32** | **31** (33이었다가 거리 항목 2개를 뺐다 — 항목 V) |
| §2 입력 | `SuitabilityInput.certifications` | `types.ts`에 없다. v5에서 Layer C와 함께 빠졌다 |
| §8 골든 목록 | `coverage-cap` `coverage-boundary` `layer-c-guard` `layer-c-cap` | 파일 4개 모두 삭제됨 |

**직접 반영한 것 둘.** 나머지와 달리 이 둘은 **그대로 두면 다음 사람이 삭제된 규칙을 되살린다** — `00_README.md`가 예외로 인정하는 바로 그 경우다.

- §3의 `unknown → 0.35` 행
- §6.4의 규칙 4b (`coverage < 0.65` 상한)

**나머지는 고치지 않았다.** 정책 버전 줄, 항목 수, 입력 계약, 골든 목록은 사람이 소유하는 문서의 본문이고, 여기 적어 두는 것으로 충분하다.

---

## V. 응급실·자동심장충격기 거리는 항목이 될 수 없다 **[실측 · 판단] ★**

`05_ingest.md` §5.7과 `06_suitability.md` §3이 `emergency_distance`·`aed_distance`를 「상황」 축의 항목으로 두고, 수집기가 직선거리를 임계값에 대어 `supported`/`partial`/`unsupported`로 바꾸게 했다 — 응급실 500m·1km, 자동심장충격기 300m·1km.

**항목은 「가능/일부/불가」로 떨어져야 하는데, 응급의료가 불가해지는 거리라는 것은 없다.** 도착 시간은 도로·구급차·그 병원이 볼 수 있는 질환이 함께 정한다. 이 임계값들이 어디에서 왔는지는 스펙에도 코드에도 적혀 있지 않다.

그 결과 화면에 나온 것: **공산성의 응급실 거리 1,627m(직선)에 빨간 ✕와 「나쁨」.** 공주의료원은 공주 시내에 있고, 산성에서 1.6km는 이 지역에서 이상한 거리가 아니다. 안전에 관한 판단을 거리 하나로 내리고, 그것을 점수에서 감점까지 한 것이다.

**코드 대응:** 두 항목을 카탈로그에서 **뺐다**(33 → 31). 거리 자체는 잃지 않았다 — 관광지 화면의 「가까운 안전시설」이 `content/facilities.json`에서 직접 읽어, 기관명·출처·「직선거리」와 함께 그대로 보여준다. 판정에는 들어가지 않는다는 문장을 그 절에 적었다.

**수정할 곳:** `05_ingest.md` §5.7의 두 행, `06_suitability.md` §3의 「상황」 축 행(5 → 3)과 §4.2·§4.3의 해당 행.

---

## W. `BARRIER_NOUN`에 머리 명사 `턱`이 없었다 **[실측 · 스펙에 직접 반영] ★**

`BARRIER_NOUN`이 **합성어 `문턱`만 적고 머리 명사 `턱`을 빠뜨렸다.** KTO는 둘 다 쓴다.

```
주 출입구는 턱이 없어 휠체어 접근 가능함   →  unsupported
```

문장이 「가능함」으로 끝나는데 판정은 「불가」다. `없어`가 붙을 장애물이 없어서, 부정이 함수 아래쪽 문장 전체 폴백으로 흘러가 **시스템에서 가장 무거운 판정**을 받았다.

```ts
const BARRIER_NOUN = /(단차|턱(?!없)|계단|장애물|급경사|경사(?!로)|돌길|자갈|비포장|협소|좁음)/g;
```

**부정 전망 `(?!없)`이 같이 필요하다.** 「턱없이·턱없어·턱없음·턱없다」는 장애물 + 부정이 아니라 **한 낱말인 부사**(한참 모자란)이고, 편의시설 이야기에 실제로 쓰인다. 전망 없이 `턱`만 넣으면 `턱없이 부족한 편의시설`이 「장애물이 없다」로 파싱돼 경로 항목에서 **`supported`로 발행된다** — 실측했다.

전망을 어미 둘(`없[이는]`)로만 좁혔던 판(이번 회차 중간 상태)에서는 `편의시설이 턱없어 불편함`이 다시 `supported`였다. 어간 전체(`없`)를 막아야 한다. 대가는 띄어쓰기 없이 쓴 `턱없음`(단차 없음의 뜻)이 장애물로 읽혀 `unsupported`가 되는 것인데, **안전한 방향**이고 KTO가 실제로 쓰는 띄어쓴 형태(`턱 없음`, `턱이 없어`)는 그대로 잡힌다.

**부정 폴백은 문장 전체로 둔다.** 이번 회차에 한 번 국소화했다가(`hasUnnegatedPresence`가 있으면 `unknown`) 되돌렸다. KTO가 확인된 부재를 적는 가장 흔한 방식이 **대체 수단을 같은 문장에 붙이는 것**이기 때문이다.

```
장애인 화장실 없음. 인근 공중화장실 이용 가능.   →  국소화하면 unknown
엘리베이터는 없으나 1층만 관람 가능              →  국소화하면 unknown
휠체어 대여 없음. 유아차는 대여 가능.            →  국소화하면 unknown
```

셋 다 **확인된 부재**인데 「확인 필요」가 된다. 09번 §1이 적어 둔 그 실패 — 「확인된 부재를 모름으로 읽으면 휠체어 이용자를 못 들어가는 건물로 보낸다」 — 가 그대로 돌아온다. 국소화를 정당화하던 유일한 문장(`대중교통 이용가능 : … 저상버스 없음.`)은 **X에서 `publictransport`를 항목에서 빼면서 이 함수에 도달할 수 없게 됐다.** 같은 증상을 두 번 고쳤고, 두 번째가 반창고였다.

**실측한 영향.** 저장된 403건을 새 함수로 다시 판정했더니 **정확히 1건**이 바뀌었다 — `서동공원과 궁남지`의 `출입통로`가 `unsupported` → `supported`. 그 한 건이 라벨을 뒤집는다: 이 관광지는 **13곳 중 유일한 「다른 곳 권장」**이었고, 그 라벨이 오직 이 문장에 걸려 있었다. 지금은 「주의 필요」다.

`대체추천`이 죽지는 않았다 — 부소산성이 시각장애 조건에서 점자블록 부재(`unsupported`, 확인된 부재)로 유지된다.

**스펙에 직접 반영했다.** `05_ingest.md` §4.2가 이 함수를 통째로 인용하고 있어서, 고치지 않으면 **버그가 규칙으로 적힌 문서**가 남는다. `00_README.md`가 예외로 인정하는 그 경우다.

**`04_open_items.md` §1.2가 예고한 것이 이것이다** — 「장애물 명사 사전에 빠진 단어」. 144문장 눈 대조는 아직 안 했지만, 원문이 붙은 48건을 전수 검사해 이 1건만 나왔다.

---

## X. `publictransport`는 항목이 될 수 없다 **[실측 · 판단] ★**

`06_suitability.md` §3과 `05_ingest.md` §4.1이 `public_transport`를 「이동」 축의 점수 항목으로 둔다. 항목은 가능/일부/불가로 떨어져야 한다.

**공주시 51곳 + 부여군 25곳, 무장애 등록 76곳의 `detailWithTour2`를 전수 조회했다.** `publictransport`는 **22곳(29%)에 채워져 있다** — 28개 필드 중 5번째로 많이 채워진, 살아 있는 필드다. 우리 13곳만 0건이다.

그런데 값이 이렇게 생겼다.

```
공주역(고속철도)에서 산성동/시내버스정류장으로 201,202번 버스 하차 후 도보 30분
5분거리 버스정류장에 610, 611, 770, 772번 버스 하차
대중교통 이용가능 : 사기소 정류장.저상버스 없음.
```

**길 안내 문장이다.** 「도보 30분」은 가능도 불가도 아니다. 실제 값 8개를 `resolveStatus`에 넣으니 **6개가 `unknown`**이었고 — 이건 올바른 거절이다 — **1개는 거꾸로** 나왔다(항목 W).

응급실 거리(항목 V)와 **같은 구조의 오류**다. 판정을 낼 수 없는 것에 판정 자리를 만들어 뒀다.

**코드 대응:** 카탈로그에서 뺐다(31 → 30, KTO 24 → 23). 문장은 `pois[].etcNotes`가 나른다 — 「무장애 항목으로 나누지 못한 원문 설명입니다. 점수에는 반영하지 않습니다」라고 이미 적혀 있는 자리이고, 새 구조를 만들지 않았다.

**같이 드러난 것:** `KTO_ETC_FIELDS` 네 개도 **한 번도 수집된 적이 없다.** 이 필드들은 `detailWithTour2`에서 오는데 그건 `accessibility` 단계가 부르고, `etcNotes`는 `pois` 단계가 만든다. 단계가 어긋나 수집 코드가 아예 없었는데, **실행 로그는 「\*etc 필드는 pois[].etcNotes로 나른다」고 찍고 있었다.** 4곳에 읽히지 않은 원문이 있다 — 공산성·부소산성의 경로 제약, 구드래조각공원의 점자·청각·기저귀교환대, 그리고 궁남지의 `지붕이 설치된 윈두막이 있음`. 마지막 것은 13곳 전부 미확인인 `그늘·실내 휴게` 그 자체다.

`buildPois`가 `getBarrierFreeDetail`을 직접 부르게 했다(실행당 13콜 추가, 오퍼레이션당 1,000/일 한도).

**수정할 곳:** `05_ingest.md` §4.1의 `public_transport` 행(반영함), `06_suitability.md` §3의 「이동」 축 행(4 → 3)과 §4.3의 해당 행.

---

## Y. `korean.visitkorea.or.kr`의 무장애 「없음」은 값이 아니라 기본값이다 **[실측] ★**

대한민국 구석구석의 관광지 상세 페이지에는 무장애 픽토그램 블록이 있다. **서버가 내려주는 HTML에는 12개가 전부 `없음`으로 박혀 있고**(`<img class="img_dummy" src="dummy_icon_bfreesvc.png">`), 자바스크립트가 나중에 덮어쓴다. curl·검색 API·일반 스크레이퍼는 전부 가짜 블록을 받는다.

**브라우저로 렌더링해도 끝이 아니다.** 렌더링된 값에서도 `없음`은 「확인된 부재」가 아니라 「이 자리에 줄 것이 없음」이다. KTO 자신의 API와 나란히 놓고 확인했다.

```
구드래조각공원 (contentId 126699)
  detailWithTour2  braileblock = 점자블록 있음(주차장 진입로 및 화장실 입구 등 일부 점형블록 설치)
  구석구석 렌더링    시각장애인 편의서비스 없음

고마나루 (contentId 2035859)
  detailWithTour2  route · restroom · braileblock  세 개만 돌아온다
  구석구석 렌더링    엘리베이터·주차·휠체어·유아차·수유실·출입구 전부 「없음」
```

**값이 채워져 있는 필드 위에도 `없음`을 찍는다.** 그러므로 이 페이지에서 읽은 `없음`으로 `unsupported`를 발행하면 안 된다.

- **읽어도 되는 것:** `access.visitkorea.or.kr`(열린관광 모두의 여행). 서버가 값을 내려준다.
- **읽으면 안 되는 것:** `korean.visitkorea.or.kr`의 무장애 블록. 어떤 방법으로 읽든.
- **`있음`은 어떤가.** 이 조사에서는 나오지 않았다. 기본값이 `없음` 한 방향이므로 `있음`은 실제 값일 가능성이 높지만, 확인하지 않았다.

이 항목으로 이번 회차에 **9건을 발행 직전에 되돌렸다**(13번 §4.3). 그중 하나는 KTO에 필드 자체가 없는 자리에 `supported`를 찍고 있었다.
